...
import { follow, get_User } from "../utils/ao";
...
function User() {
  ...
  const [followers, setFollowers] = useState<Array<{ address: string }>>([]);
  const [following, setFollowing] = useState<Array<{ address: string }>>([]);
  const [articles, setArticles] = useState<Array<string>>([]);
  const __username = useAccount((state) => state.username);
  const { username } = useParams();
  const navigate = useNavigate();
  const addresss = useAddress((state) => state.address);
  ...
  return (
    <>
      <div className="container mx-auto lg: w-1/2">
        <div className="max-w-2xl mx-4 sm:max-w-sm md:max-w-sm lg:max-w-sm xl:max-w-sm sm:mx-auto md:mx-auto lg:mx-auto xl:mx-auto mt-16 bg-white shadow-xl rounded-lg text-gray-900">
          ...
          <div className="text-center mt-2">
            ...
            <p className="text-gray-400">@{_username}</p>
          </div>
          ...
          {__username === _username ? null : following.filter(
              (e) => e.address === addresss
            ).length ? (
            ...
            <div className="p-4 border-t mx-8 mt-2">
              <button
                ...
                onClick={() => follow(_username).then().catch(console.log)}
              >
                Follow
              </button>
            </div>
      ...
      <div className="mt-5">
        {articles.length ? (
          <>
            <ArticleList address={articles} username={_username} />
          </>
        ) : (
          <>
            <div className="w-full h-screen flex flex-col lg:flex-row items-center justify-center space-y-16 lg:space-y-0 space-x-8 2xl:space-x-0">
              <div className="w-full lg:w-1/2 flex flex-col items-center justify-center lg:px-2 xl:px-0 text-center">
                ...
                <p className="text-lg md:text-xl lg:text-2xl text-gray-500 my-12">
                  Sorry, the articles you are looking for cannot be found
                </p>
              </div>
        ...
        <div className="gap-2 grid grid-cols-2 sm:grid-cols-4"></div>
      </div>
    </>
  );
}
